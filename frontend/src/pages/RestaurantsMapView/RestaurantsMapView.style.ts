...
export const useStyles = createUseStyles((theme: AppTheme) => ({
  ...
  restaurantList: {
    ...
    width: "85vw",
    margin: "auto",
    marginBottom: `calc(-10 * ${theme.spacing.large} + 100vh - ${MAP_HEIGHT} - ${theme.headerHeight} - ${PAGINATION_HEIGHT} - ${DISTANCE_HEIGHT})`,
  },
  restaurantListDesktop: {
    marginTop: `calc(${theme.headerHeight} + ${theme.spacing.medium})`,
    width: DESKTOP_MASTER_WIDTH,
    position: "relative",
    marginBottom: `calc(-10 * ${theme.spacing.large} + 100vh - ${theme.headerHeight} - ${PAGINATION_HEIGHT} - ${DISTANCE_HEIGHT})`,
  },
